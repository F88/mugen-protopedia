...
export class PrototypeMapStore {
  ...
  setAll(
    ...
    if (approxSizeBytes > this.maxPayloadSizeBytes) {
      this.logger.warn(
        ...
        'Snapshot skipped: payload exceeds maximum size',
      );
      return null;
    }
    ...
    this.logger.info(
      ...
      'PrototypeMapStore snapshot updated',
    );
  ...
  clear(): void {
    ...
    this.logger.info({ previousSize }, 'PrototypeMapStore cleared');
  }
  ...
  runExclusive(task: RefreshTask): Promise<void> {
    ...
    this.refreshPromise = (async () => {
      try {
        ...
        this.logger.error({ error }, 'PrototypeMapStore refresh task failed');
        throw error;
      } finally {
...
/** Singleton instance used by server actions. */
export const prototypeMapStore = new PrototypeMapStore();
